			...
			try{
				if(parent.document.getElementById('head').allowTransparency==true){
					if(top!=self){
						...
						try{
							parent.document.getElementById('headerdiv').style.zIndex =600000;
						}
						catch(err){
...
function do_bringtobottom(){
	if(!is_safari){
		try{
			parent.document.getElementById('headerdiv').style.zIndex =600000;
			document.getElementById("header").style.height="57px";
		}
		catch(err){
...
try{
		if(parent.document.location.href.indexOf("ebisearch")!=-1){
